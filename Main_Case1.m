...
if(isave>0) saveDir=sprintf('Saved/%s_noise_%d', date(),round(noise*100)); [suc,mesg,msgid]=mkdir(saveDir);   end

%PlotAll(.1,kpowV,'Saved/15-Jul-2016',1,1); return;

randValue=2*(rand(nscreen,1)-.5); % Random value in [-1:1]
%--------------------------------------------------------------
for kpow=kpowV % kpow=alpha
 ...
 for Rytov=RytovV; 
  Cn2= Rytov2Cn2(Rytov,wvl,Dz,kpow); Cn2_s=Cn2*(1+noise*randValue); 
  csvwrite(sprintf('%s/Cn2_%.1f_%.1f.csv',saveDir,6*kpow,Rytov),Cn2_s,0,0); 
  tic
  ...
  if(isave==2) 
      ...
      fprintf(fileID2,'itrial, Rytov, Cn2, r0, n(I<1%%), n(%%), varI, minI, maxI, 6*kpow\n'); 
  end;
  for itrial=1:ntrial 
    Uin=ones(N,N); g=Uin; G=zeros(N,N); phz=G;  
    for idx=1:nscreen-1
      %[phz, r0, Cn2]=Phase_TurbAlpha(Rytov,wvl,N,deltax,deltaz, Dz,Lout,Lin,kpow); 
      [phz, r0]=Cn2ToPhase(Cn2_s(idx),wvl,N,deltax,deltaz, Lout,Lin,kpow); 
    ...
    Uout = g; Asq=abs(Uout).^2; varI=var(Asq(:)); meanI=mean(Asq(:)); minI=min(Asq(:)); maxI=max(Asq(:)); 
    ...
    fprintf('Trial=%1d, Rytov=%.1f, Cn2=%.1e, r0=%.1f, n(Iz<1%%Imax)=%d, nIz%%=%.1f, varI=%.2f, minI=%.1e, maxI=%.1e, varI/Rytov=%.2f\n',...
        itrial, Rytov,Cn2,r0,nIz,nIzp,varI,minI,maxI, varI/Rytov);
    if(isave==2) fprintf(fileID2,'%d, %.1f, %.1e, %.2f, %d, %.2f, %.2f, %.1e, %.1e, %.2f\n',itrial, Rytov,Cn2,r0,nIz,nIzp,varI,minI,maxI,kpow*6); end
  end; % ntrial
